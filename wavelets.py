...
(26/11/11): AMe: introduce 'smallestscale' 
"""
...
class Cwt:
    ...
    def __init__(self, data, largestscale=1, notes=0, order=2, \
                 scaling='linear', smallestscale=2):
        """
        ...
        smallestscale: smallest scale (default=2) in 1/sample
        """
        ndata = len(data)
        self.order=order
        self.scale=largestscale
        self._setscales(ndata,largestscale,notes,scaling,smallestscale)
    ...
    def _setscales(self,ndata,largestscale,notes,scaling, smallestscale=2):
        """
        ...
        if scaling=="log":
            ...
            noctave=self._log2(ndata/largestscale-smallestscale )            
            self.nscale=notes*noctave
            self.scales=NP.zeros(self.nscale,float)            
            self.scales = NP.arange(self._log2(smallestscale),\
                                    self._log2(ndata/largestscale),\
                                    (self._log2(ndata/largestscale)-\
                                     self._log2(smallestscale))/\
                                    float(self.nscale))
            self.scales = 2**self.scales
        elif scaling=="linear":
            self.scales=NP.arange(float(smallestscale),\
                                  float(ndata/largestscale),\
                                  0.5)
...
class Morlet(Cwt):
    ...
    Morlet wavelet
    """
    _omega0=8.0 # N fringes per packet?
    fourierwl=4*NP.pi/(_omega0+ NP.sqrt(2.0+_omega0**2))
    #fourierwl=2/(_omega0+ NP.sqrt(2.0+_omega0**2))
    def wf(self, s_omega):
        ...
        xhat=NP.pi**(-0.25)*\
              NP.sqrt(1+NP.exp(-self._omega0**2) - \
                      2*NP.exp(-0.75*self._omega0**2))*\
              ( NP.exp(-(s_omega-self._omega0)**2/2.0)-\
                NP.exp(-0.5*self._omega0**2-0.5*s_omega**2))*H
        return xhat
...
class DOG(Cwt):
    ...
    def wf(self, s_omega):
        ...
        m=self.order
        dog=1.0J**m*s_omega**m* NP.exp(-s_omega**2/2)/\
             NP.sqrt(gamma(self.order+0.5))
        return dog
